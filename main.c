  ...
  while (err != ERR_EOF) {
    ...
    case TypeData: {
      ...
      if (line->flags & FlagSymbolDeclaration) {
        Symbol *sym = newSymbol(line->label, memory->data_counter, false, false,
                                SymbolSection_Data);
        err = symtab->insert(symtab, sym);
      }
    ...
    case TypeCode:
      if (line->flags & FlagSymbolDeclaration) {
        Symbol *sym = newSymbol(line->label, memory->instruction_counter, false,
      ...
      /* count instructions here so we know what the final code size is before
       * second pass */
